...
export default function QuizTakePage() {
  ...
  return (
    <QuizSecurityWrapper enabled={true} maxViolations={3} onAutoSubmit={executeSubmit}>
      <div className="bg-background text-on-surface font-body-md antialiased min-h-screen relative w-full -ml-4 sm:-ml-0 select-none">
        ...
        <div className="pt-20 pb-28 md:pb-24 px-4 md:px-margin-desktop w-full max-w-container-max mx-auto flex flex-col lg:flex-row gap-gutter">
          ...
          <section className="flex-1 flex flex-col gap-6">
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold transition-all border ${
                      flaggedQuestions[currentQuestion.questionId]
                        ...
                        : "text-outline border-outline-variant/40 hover:text-primary hover:border-primary"
                    }`}
                  >
                    <span 
                      className="material-symbols-outlined text-[18px]"
                      style={{ fontVariationSettings: flaggedQuestions[currentQuestion.questionId] ? "'FILL' 1" : "'FILL' 0" }}
                    >
                      bookmark
                    </span>
            ...
            <div className="p-base bg-surface-container-low rounded-lg border border-dashed border-outline-variant flex items-start gap-3 mt-auto">
              ...
              <p className="text-caption text-on-surface-variant">Hệ thống sẽ tự động gửi bài làm của bạn khi hết thời gian quy định hoặc vi phạm quy chế quá số lần cho phép.</p>
            </div>
          </section>
            ...
            <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant shadow-sm sticky top-24">
              ...
              <div className="grid grid-cols-5 sm:grid-cols-8 lg:grid-cols-5 gap-2 mb-6">
                {questions.map((q, idx) => {
                  ...
                  let btnClass = "relative h-10 w-full rounded flex items-center justify-center text-label-md transition-all cursor-pointer ";
                  if (isCurrent) {
